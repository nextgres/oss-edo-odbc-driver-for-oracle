...
//   }
  ...
  // Set reuse for connection if we are in the statement pool
...
ODBCStatement::SQLExecute()
{
  ...
  retCode = m_IPD->BindAllParams(m_APD);
  ...
  if((m_connection->GetAutoCommitMode() == SQL_AUTOCOMMIT_ON) && (m_inNeedForData == false))
  {
    transMode = OCI_COMMIT_ON_SUCCESS;
  }
                            ...
                            ,SQLUSMALLINT* p_status_ptr)
{
      ...
      for(i=0;i<recCount;++i)
		    ...
		    if(lpIRColumn && lpARColumn && lpARColumn->m_desc_type && lpARColumn->m_desc_data_ptr)
        {
          ...
          if(m_ARD->GetArraySize())
          {
            ...
            dataPointer = (char*)dataPointer + (row * m_ARD->GetArraySize());
          }
          else
          {
            // COLUMNWISE binding (calculate next array element)
            dataPointer = (char*)dataPointer + lpARColumn->m_desc_length;
          }
...
#endif
    // Next row to fetch for ROW-WISE / COLUMN-WISE binding
    ++row;
    if(p_status_ptr)
    {
      ++p_status_ptr;
  ...
  while(row < m_ARD->GetArraySize());

  // STEP 6: Do the status for ungotten rows
  if(p_status_ptr)
  {
    while(row++ < m_ARD->GetArraySize())
    {
...
ODBCStatement::SQLParamData(SQLPOINTER* Value)
{
  ...
  // Must only be called after a SQLExecute returns SQL_NEED_DATA
  if(m_cancel)
  {
    // Statement canceled
  ...
  // COMPLETE. 
  if(m_inNeedForData == false)
  {
     return SQL_SUCCESS;
  }
  // End criterium of all AT_EXEC Columns
  ODBCItems* check = m_APD->GetItem((short)m_needDataColumn);
  while(check)
  {
    if(*check->m_desc_indicator_ptr == SQL_DATA_AT_EXEC ||
       *check->m_desc_indicator_ptr <= SQL_LEN_DATA_AT_EXEC_OFFSET)
    {
      // One more to come!
      break;
    }
    check = m_APD->GetItem((short)++m_needDataColumn);
  }
  if(check == NULL)
  {
    // Ready with all piecewise insertions
    m_inNeedForData  = false;
    m_needDataColumn = 0;
    return SQL_SUCCESS;
  }
  OCIBind* bindPtr;
  ub4 type  = OCI_HTYPE_BIND;
  ub1 inout = OCI_PARAM_IN;
  ub4 rowNum,arrayNum;
  ub1 piece = 0;
  if(OCIStmtGetPieceInfo(m_hStmt
                        ,m_ociError.GetOwnerErrorObject()
                        ,(dvoid**)&bindPtr
                        ,&type
                        ,&inout
  ...
  OracleItems* record = NULL;
  ODBCItems*   item   = NULL;
  short column = m_IPD->FindBindHandle(bindPtr);
  if(column >= 0)
  {
    record = m_IPD->GetItem(column);
    item   = m_APD->GetItem(column);
  }
  if(column < 0 || record == NULL || item == NULL)
  {
    // Invalid descriptor index
    m_ociError.AddError("07009");
    return SQL_ERROR;
  ...
  // Get the bounded column info
  *Value = item->m_desc_data_ptr;
  return SQL_NEED_DATA;
}
                         ...
                         ,SQLLEN     StrLen_or_Ind)
{
  ...
  ociRes = OCIStmtSetPieceInfo(record->OCIObject.lpBind
                              ,OCI_HTYPE_BIND
                              ,m_ociError.GetOwnerErrorObject()
                              ,Data
                              ,&amount
  ...
  ociRes = OCIStmtExecute(GetDBConnection()->GetOCIConnection()
                         ,m_hStmt
                         ,m_ociError.GetOwnerErrorObject()
                         ,1  // ALways others (no SELECT) for piecewise insert
                         ,0
                         ,NULL
                         ,NULL
                         ,OCI_DEFAULT);
  if(ociRes == OCI_NEED_DATA)
  {
    // Piece inserted. Do bookkeeping
    record->m_pendingLength -= amount;
    if(m_needPiece == OCI_FIRST_PIECE)
    {
      m_needPiece = OCI_NEXT_PIECE;
    }
    if(record->m_pendingLength <= 0)
    {
      // This column is ready. Go to the next column 
      // Flag success so SQLParamData gets called
      ++m_needDataColumn;
      return SQL_SUCCESS;
    }
  }
  else
  {
    if(ociRes == OCI_SUCCESS)
    {
      // Complete/all pieces inserted
      m_inNeedForData = false;
      if(m_connection->GetAutoCommitMode() == SQL_AUTOCOMMIT_ON)
      {
        // Do the postponed commit from the original SQLExecute
        ociRes = OCITransCommit(GetDBConnection()->GetOCIConnection()
                               ,m_ociError.GetOwnerErrorObject()
                               ,OCI_DEFAULT);
      }
    }
